...
**Commits landed:** 4 of 9 (`3b9ccef`, `64c817d`, `021e87a`, commit 4 below).
...
| 4   | 🟡     | (pending) | Moved `bg/constants.js` (URL bases, `GRADE_MAP`, `SUBJECT_MAP`), `bg/cache.js` (`CACHE_TTL` + `cacheGet/Set/Age`), `bg/session.js` (`withSessionLock` module singleton), `bg/bannerApi.js` (`getTerms`, `getCurrentTerm`, `searchCourse`, `searchCoursesBySubjects`), `bg/prereqs.js` (`checkPrereqs`, `getCourseDescription`). `background.js` gained named ES imports; removed definitions dropped. 132 tests green, Node SW-module-graph smoke (FIFO + cache round-trip) ✓. | **Pending manual smoke: eligible list fills <3s.**                               |
| 5   | ⬜      | —         | Move `bg/studentInfo.js` (RequirementGraph + `expandAuditWildcards`) + `bg/registration.js` (`registrationHistory` + `/saml/login`).                                                                                                                                     | Tests green + closed-term schedule smoke + `auditDiagnostics.parity` spot-check. |
...
Task for next chat: **Commit 4** — move `bg/constants.js`, `bg/cache.js`, `bg/session.js`, `bg/bannerApi.js`, `bg/prereqs.js`. Preserve `withSessionLock` as a module singleton, preserve `searchCoursesBySubjects` + `subjectSearch|v2|` cache keys, route prereqs/descriptions through `self.BPPerf.mapPool`. End with tests green + eligible-list smoke <3s.
