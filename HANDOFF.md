...
> is **code-complete** through commit `998e63f` (`extension/tab/*` per Deviation B). **Commit 9**
> (this doc pass) restructured documentation: router `CLAUDE.md`, `docs/architecture.md`,
> `docs/invariants.md`, `docs/file-map.md`, `docs/open-bugs.md`, plus `bug9` / `bug10`
> deferred diagnoses. **No runtime behavior changes** in commit 9.
...
| 4a–5 | Advising + planner | ⬜ — see `docs/advising-flow.md` |
...
1. **Merge or PR** `refactor-on-main` → `main` when you are ready (Auto: fresh chat,
  green `node tests/unit/run.js`, Chrome smoke on auth → term → eligible → AI → lock/save
  if anything non-doc touched).  
2. **Bug 7** — registration restrictions (Opus/API) — file `docs/bug7-*` first.  
...
- Earlier: SW ES module, leaf `bg/*` extraction — see `docs/refactor-on-main-plan.md`

**`main` (post-`6d5c80e` merge)** — D19, A1+B perf, D17, phase work — see `git log main`
for full history.

**Deferred Refactor-only fixes** recorded as `docs/bug9-*` and `docs/bug10-*`; see
D24 in `docs/decisions.md` for superseded Refactor tickets.
